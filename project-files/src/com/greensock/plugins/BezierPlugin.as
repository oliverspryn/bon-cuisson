 ...
 * 
 * 		<li><code>"thruFast"</code> - the same as <code>"thru"</code> except that it uses a less complex 
 * 				algorithm for the initial plotting of the Bezier through the supplied values. The "thruFast"
 ...
 * 				than <code>"thruFast"</code>. In terms of calculation expense, "thruFast" is only about 15-20% faster 
 * 				on the initial setup (when the tween begins), but then every update during the tween the speed is 
 * 				identical, so overall improvement is negligible. The primary reason the <code>"thruFast"</code> 
 * 				option is available is to offer a different style for drawing the Bezier through the supplied values.
 * 				</li>
 * 
 ...
 * 
 * 		<li><strong>type</strong> : String (default:<code>"thru"</code>) - Either <code>"thru", "thruFast", "soft", "quadratic",</code>
 ...
 * 				set <code>autoRotate:true</code>. However, for more advanced controls, you can define <code>autoRotate</code>
 * 				as an array. In order to adjust a rotation property accurately, the plugin needs 5 pieces of information:
 * 				<ol>
 ...
 */
	public class BezierPlugin extends TweenPlugin {
		...
		override public function _onInitTween(target:Object, value:*, tween:TweenLite):Boolean {
			...
			this._autoRotate = autoRotate ? (autoRotate is Array) ? autoRotate as Array : [["x","y","rotation",0]] : null;
			...
			this._beziers = (vars.type !== "cubic" && vars.type !== "quadratic" && vars.type !== "soft") ? bezierThrough(values, isNaN(vars.curviness) ? 1 : vars.curviness, false, (vars.type === "thruFast"), vars.correlate || "x,y,z", prepend) : _parseBezierData(values, vars.type, first);
 ...
 for (var i:int = 0; i &lt; bx.length; i++) {
 	g.curveTo(bx[i].b, by[i].b, bx[i].c, by[i].c);
		 ...
		 * @param fast if <code>true</code>, a faster algorithm will be used for calculating the control points which can be less aesthetically pleasing in situations where there are large differences in the spaces or angles between the values provided (the curves are more likely to get kinks or harsh angles)
		 ...
		 */
		public static function bezierThrough(values:Array, curviness:Number=1, quadratic:Boolean=false, fast:Boolean=false, correlate:String="x,y,z", prepend:Object=null):Object {
			var obj:Object = {},
				cnt:int = 0,
			...
			while (--i > -1) {
				p = props[i];
				_calculateControlPoints(obj[p], curviness, quadratic, fast, _corProps[p]); //this method requires that _parseAnchors() and _setSegmentRatios() ran first so that _r1, _r2, and _r3 values are populated for all properties
			}
			return obj;
		}
		...
		public static function _parseBezierData(values:Array, type:String, prepend:Object=null):Object {
			...
			if (values == null || values.length < 3) { throw new Error("invalid Bezier data"); }
		 ...
		 */
		protected static function _calculateControlPoints(a:Array, curviness:Number=1, quad:Boolean=false, fast:Boolean=false, correlate:Boolean=false):void {
			...
			for (i = 0; i < l; i++) {
				...
				if (correlate) {
					r1 = _r1[i];
					r2 = _r2[i];
					tl = ((r2 + r1) * curviness * 0.25) / (fast ? 0.5 : _r3[i] || 0.5);
					m1 = p2 - (p2 - p1) * (fast ? curviness * 0.5 : tl / r1);
					m2 = p2 + (p3 - p2) * (fast ? curviness * 0.5 : tl / r2);
